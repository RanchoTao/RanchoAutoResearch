...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if hitl_mode:
            ...
            # Presets describe intervention behavior.  Preserve an explicit
            # config-file cost guard when the preset does not define one.
            configured_hitl = getattr(config, "hitl", None)
            configured_budget = getattr(configured_hitl, "cost_budget_usd", 0.0)
            if (
                hitl_config.cost_budget_usd <= 0
                and configured_budget > 0
            ):
                import dataclasses as _dc_hitl

                hitl_config = _dc_hitl.replace(
                    hitl_config,
                    cost_budget_usd=configured_budget,
                )
