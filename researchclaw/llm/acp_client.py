...
class ACPClient:
    ...
    def _extract_response(raw_output: str | None) -> str:
        ...
        Supports the JSON-RPC stream emitted by modern acpx
        (``--format json --json-strict``) and the legacy text format.
        ...
        # Modern acpx emits one JSON-RPC object per line.  Codex streams the
        # final answer through ``agent_message_chunk`` updates; concatenating
        # only chunks marked ``phase=final_answer`` avoids warnings and
        # reasoning preambles.  Other ACP agents may omit that phase metadata,
        # so retain a secondary collection as a compatibility fallback.
        final_chunks: list[str] = []
        message_chunks: list[str] = []
        parsed_json_line = False
        for raw_line in raw_output.splitlines():
            ...
            codex_meta = meta.get("codex") if isinstance(meta, dict) else None
            phase = codex_meta.get("phase") if isinstance(codex_meta, dict) else None
        ...
        if parsed_json_line and message_chunks:
            return "".join(message_chunks).strip()
