...
def _execute_literature_screen(
    ...
    *,
    ...
    # Rank before applying the prompt-size cap.  Stage 4 merges providers and
    # sorts globally by citation count, which can put highly cited papers from
    # unrelated domains ahead of genuinely relevant work.  IDF downweights
    # broad topic tokens (for example, "model" or "learning") while the title
    # multiplier favors papers whose main subject matches the research topic.
    # Keeping this deterministic also makes the parse-failure fallback below
    # select the best available real records instead of the most cited ones.
    ...
    keyword_doc_freq = {
        kw: sum(1 for title, abstract in row_blobs if kw in title or kw in abstract)
        for kw in topic_keywords
    }
    row_count = len(filtered_rows)
    for row, (title, abstract) in zip(filtered_rows, row_blobs, strict=True):
        relevance = 0.0
        for kw in topic_keywords:
            if kw not in title and kw not in abstract:
                continue
            idf = math.log((row_count + 1) / (keyword_doc_freq[kw] + 1)) + 1.0
            relevance += idf * (3.0 if kw in title else 1.0)
        row["topic_relevance_score"] = round(relevance, 3)
    filtered_rows.sort(
        key=lambda row: (
            float(row.get("topic_relevance_score", 0.0)),
...
def _execute_knowledge_extract(
    ...
    *,
    ...
    if llm is not None:
        ...
        # A single response for a serious 15-30 paper review commonly exceeds
        # the ACP agent's one-turn output limit and leaves truncated, invalid
        # JSON.  Extract in small batches and validate cardinality so downstream
        # synthesis never runs on silent template/fabricated cards.
        source_rows = _parse_jsonl_rows(shortlist)
        ...
        for batch_start in range(0, len(source_rows), batch_size):
            ...
            batch_text = "\n".join(
                json.dumps(row, ensure_ascii=False) for row in batch
            )
            sp = _pm.for_stage(
                "knowledge_extract",
                evolution_overlay=_overlay,
                shortlist=batch_text,
            )
            resp = _chat_with_prompt(
                llm,
                sp.system,
            ...
            (stage_dir / f"raw-response-batch-{batch_num}.txt").write_text(
                resp.content,
                encoding="utf-8",
            )
            payload = _safe_json_loads(resp.content, {})
            ...
            if isinstance(payload, dict) and isinstance(payload.get("cards"), list):
                batch_cards = [
                    item for item in payload["cards"] if isinstance(item, dict)
                ]
            if len(batch_cards) != len(batch):
                failed_batches.append(batch_num)
                logger.warning(
                    "Stage 6: batch %d returned %d cards for %d papers",
                    batch_num,
                    len(batch_cards),
                    len(batch),
                )
        ...
        if failed_batches:
            (stage_dir / "knowledge_meta.json").write_text(
                json.dumps(
                    {
                        "outcome": "invalid_or_incomplete_acp_batches",
                        "input_papers": len(source_rows),
                        "cards_extracted": len(cards),
                        "failed_batches": failed_batches,
                        "batch_size": batch_size,
                        "note": (
                            "Knowledge extraction paused instead of creating "
                            "template cards. Inspect raw-response-batch-*.txt."
                        ),
                    },
                    indent=2,
                ),
            ...
            return StageResult(
                stage=Stage.KNOWLEDGE_EXTRACT,
                status=StageStatus.PAUSED,
                artifacts=("knowledge_meta.json", "raw-response-batch-*.txt"),
                error="ACP knowledge-card extraction returned incomplete JSON batches",
                evidence_refs=("stage-06/knowledge_meta.json",),
                decision="invalid_structured_output",
            )
    if not cards:
