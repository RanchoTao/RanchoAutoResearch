...
def _execute_iterative_refine(
    ...
    *,
    ...
    # DeepSeek is the experiment strategist when configured. Codex remains the
    # local code executor and receives only one approved proposal per round.
    # A missing/failed DeepSeek service explicitly falls back to the existing
    # Codex-only refinement prompt; it never fabricates a strategist result.
    deepseek_client: Any | None = None
    deepseek_session_available = False
    try:
        from researchclaw.experiment.deepseek_agent import (
            DeepSeekClient,
            DeepSeekConfig,
        ...
        if is_deepseek_available():
            ...
            deepseek_session_available = True
        else:
            logger.warning(
                "Stage 13: DEEPSEEK_AVAILABLE=false; using Codex-only fallback. "
                "Set DEEPSEEK_API_KEY to enable DeepSeek-first strategy."
            )
    except Exception as exc:  # noqa: BLE001 - availability must never fake success
        logger.warning(
            "Stage 13: DeepSeek initialization failed; using Codex-only fallback: %s",
            exc,
        )
    log["strategy"] = {
        "mode": "deepseek-first" if deepseek_session_available else "codex-only-fallback",
        "deepseek_available": deepseek_session_available,
        "codex_role": "proposal implementation, execution, validation, and fallback",
    }
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        deepseek_proposal: dict[str, Any] | None = None
        if deepseek_session_available and deepseek_client is not None:
            try:
                from researchclaw.experiment.deepseek_agent import build_research_state

                recent_experiments: list[dict[str, Any]] = [
                    *baseline_records,
                    *[
                        item
                        for item in log["iterations"]
                        if isinstance(item, dict)
                    ],
                ]
                failed_ideas = [
                    str(item["deepseek_proposal"].get("hypothesis", ""))
                    + " | "
                    + str(item["deepseek_proposal"].get("change", ""))
                    for item in log["iterations"]
                    if isinstance(item, dict)
                    and not item.get("improved", False)
                    and isinstance(item.get("deepseek_proposal"), dict)
                ]
                experiment_id = f"{run_dir.name}-stage13-iter{iteration}"
                state = build_research_state(
                    research_goal=config.research.topic,
                    metric_key=metric_key,
                    metric_direction=metric_direction,
                    current_metric=_metrics_history[-1] if _metrics_history else baseline_metric,
                    best_metric=best_metric,
                    current_files=best_files,
                    recent_experiments=recent_experiments,
                    failed_ideas=failed_ideas,
                    hardware=_load_hardware_profile(run_dir),
                    time_budget_sec=config.experiment.time_budget_sec,
                    experiment_id=experiment_id,
                    evaluator_definition=(
                        f"Run the real {config.experiment.mode} evaluator and parse "
                        f"'{metric_key}' from results.json/results.csv/stdout. "
                        f"The configured objective is to {metric_direction} it."
                    ),
                    experiment_plan=_exp_plan_text,
                )
                deepseek_dir = stage_dir / "deepseek"
                deepseek_dir.mkdir(parents=True, exist_ok=True)
                (deepseek_dir / f"state-iter{iteration}.json").write_text(
                    json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8"
                )
                if iteration == 1:
                    deepseek_proposal = deepseek_client.propose(
                        state, experiment_id=experiment_id
                    )
                else:
                    deepseek_proposal = deepseek_client.analyze(
                        state, experiment_id=experiment_id
                    )
                (deepseek_dir / f"proposal-iter{iteration}.json").write_text(
                    json.dumps(deepseek_proposal, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
                logger.info(
                    "Stage 13 iteration %d: DeepSeek proposal validated and handed to Codex executor",
                    iteration,
                )
            except Exception as exc:  # noqa: BLE001 - explicit Codex fallback
                logger.warning(
                    "Stage 13 iteration %d: DeepSeek strategy failed; using Codex-only fallback: %s",
                    iteration,
                    exc,
                )
                deepseek_session_available = False
                log["strategy"]["mode"] = "codex-only-fallback"
                log["strategy"]["deepseek_available"] = False
                log["strategy"]["fallback_reason"] = str(exc)[:1000]

        if deepseek_proposal is not None:
            # Codex gets the approved strategy and current files, but not the
            # full experimental history or an invitation to brainstorm again.
            ip_system = (
                "You are the local AutoResearch execution layer. A separate DeepSeek "
                "strategist has selected one experiment. Perform a quick repository and "
                "safety sanity check, then implement that proposal with the smallest patch. "
                "Do not brainstorm alternatives. Do not invent results. Return only updated "
                "files using ```filename:path blocks (or one Python code block for main.py)."
            )
            user_prompt = (
                "APPROVED DEEPSEEK EXPERIMENT PROPOSAL:\n"
                + json.dumps(deepseek_proposal, ensure_ascii=False, indent=2)
                + "\n\nEXECUTION CONSTRAINTS:\n"
                + f"- Primary metric: {metric_key} ({metric_direction})\n"
                + f"- Per-run time budget: {config.experiment.time_budget_sec}s\n"
                + "- Change only files named in files_to_modify.\n"
                + "- Preserve evaluator output and all do_not_change items.\n"
                + "- If the proposal plainly conflicts with the files, make no speculative "
                + "rewrite; return the current files unchanged.\n\n"
                + _exp_plan_anchor
                + _condition_coverage_hint
                + _saturation_hint
            ...
            ip_system = ip.system
            ip_max_tokens = ip.max_tokens or 8192
        ...
        if deepseek_proposal is not None and extracted_files:
            approved_files = set(deepseek_proposal["files_to_modify"])
            unapproved_files = sorted(set(extracted_files) - approved_files)
            if unapproved_files:
                logger.warning(
                    "Stage 13 iteration %d: Codex returned files outside the "
                    "DeepSeek proposal; ignoring %s",
                    iteration,
                    unapproved_files,
                )
                extracted_files = {
        ...
        iter_record: dict[str, Any] = {
            ...
            "metric": None,
            "improved": False,
            "strategy_source": "deepseek" if deepseek_proposal is not None else "codex_fallback",
        }
        if deepseek_proposal is not None:
            iter_record["deepseek_proposal"] = deepseek_proposal
        if issue_text:
