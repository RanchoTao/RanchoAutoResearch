"""Evaluate one PolyPythias run across preregistered checkpoints.

Writes after every checkpoint so an interrupted sweep is resumable. Evaluation
seeds are fixed text resamples and are not treated as independent training runs.
"""
...
def evaluate_checkpoint(model_name: str, revision: str, step: int, tokenizer,
    ...
    original_layers = model.gpt_neox.layers
    candidates = middle_layers(len(original_layers))
    ...
    for evaluation_seed in config["evaluation_seeds"]:
        ...
        for layer in candidates:
            ...
            layer_rows.append({
                "layer": layer,
                "relative_depth": layer / (len(original_layers) - 1),
                "top1_agreement": item["agree"] / tokens_seen,
                "deleted_nll": changed_nll,
...
def main() -> None:
    ...
    if args.scale not in config["scales"] or args.run_id not in config["run_ids"]:
        raise ValueError("Scale/run-id not preregistered")

    model_name = f"EleutherAI/pythia-{args.scale}m-seed{args.run_id}"
    ...
    output = ROOT / config["output_dir"] / f"pythia-{args.scale}m-seed{args.run_id}.json"
    ...
    for revision, step in zip(config["revisions"], config["checkpoint_steps"]):
        ...
        mean_agree = float(np.mean([
            layer["top1_agreement"]
            for seed in row["seed_results"] for layer in seed["layers"]
        ]))
        print(json.dumps({
            "model": model_name, "step": step, "mean_nll": mean_nll,
            "mean_agreement": mean_agree, "runtime_seconds": row["runtime_seconds"],
            "output": str(output),
        }), flush=True)
