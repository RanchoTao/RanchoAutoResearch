...
def test_acp_extracts_final_answer_from_json_rpc_chunks():
    from researchclaw.llm.acp_client import ACPClient
    ...
    assert ACPClient._extract_response(raw) == '{"ok":true}'


def test_acp_extract_response_keeps_legacy_text_compatibility():
    from researchclaw.llm.acp_client import ACPClient

    raw = "[client] initialize\nlegacy answer\n[done] end_turn\n"
    assert ACPClient._extract_response(raw) == "legacy answer"
