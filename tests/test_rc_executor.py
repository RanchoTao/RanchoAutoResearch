...
class TestIMP21_KnowledgeExtractEmptyShortlistGate:
    ...
    def test_batches_structured_extraction_and_preserves_all_cards(
        self, rc_config: RCConfig, adapters: AdapterBundle, run_dir: Path
    ) -> None:
        from researchclaw.llm.client import LLMResponse
        from researchclaw.pipeline.stage_impls._literature import (
        ...
        rows = [
            {
                ...
                "title": f"Paper {idx}",
                "abstract": "Partial observability and latent dynamics.",
        ...
        for start, count in ((0, 5), (5, 1)):
            ...
            responses.append(json.dumps({"cards": cards}))
        ...
        class SequentialLLM:
            ...
            def chat(self, messages: object, **kwargs: object) -> LLMResponse:
                _ = messages, kwargs
                payload = self.payloads[self.calls]
                self.calls += 1
                return LLMResponse(content=payload, model="test")

        llm = SequentialLLM(responses)
        stage_dir = run_dir / "stage-06"
        stage_dir.mkdir()
        result = _execute_knowledge_extract(
            stage_dir, run_dir, rc_config, adapters, llm=llm  # type: ignore[arg-type]
        )

        assert result.status == StageStatus.DONE
        assert llm.calls == 2
        assert len(list((stage_dir / "cards").glob("*.md"))) == 6

    def test_invalid_structured_extraction_pauses_without_templates(
        self, rc_config: RCConfig, adapters: AdapterBundle, run_dir: Path
    ) -> None:
        from researchclaw.pipeline.stage_impls._literature import (
            _execute_knowledge_extract,
        ...
        s5_dir.mkdir()
        (s5_dir / "shortlist.jsonl").write_text(
            json.dumps({"title": "Paper", "abstract": "Evidence"}),
            encoding="utf-8",
        )
        stage_dir = run_dir / "stage-06"
        stage_dir.mkdir()
        ...
        assert result.status == StageStatus.PAUSED
        assert result.decision == "invalid_structured_output"
        assert not list((stage_dir / "cards").glob("*.md"))
